...
3. **Agent structured outputs** — `app/agents/schemas.py`: `Citation`,
   `ChatAnswer`, and a full `AgentProposal` aligned with README §8A.4 (`proposal_id`,
   `action`, `target_doc_id`, `source_doc_ids`, `diff{}`, `draft`, `citations`,
   `evidence[]`, `confidence`, `risk_level`, `conflicts_with`, `verification{}`,
   plus Guardian fields `recommendation`, `guardian_reasoning`, `uncertainty`).
   Lean `CuratorDraft` / `GuardianReview` schemas are what the LLM actually fills;
   the graph grounds provenance fields deterministically.
...
> The implemented `AgentProposal` now matches README §8A.4 in shape (identity,
> evidence, diff, verification block). `verification` stays `null` until the
> sandbox runs against a proposal; `evidence[]` is system-populated from grounded
> retrieval rows, never the model.
...
# Agents (/chat, /propose) + AI doc summaries use Azure OpenAI env in .env:
